...
def main(args):
    ...
    # logging variables
    best_attack_optimum = 1e7
    best_attack_solution = 0
    best_test_optimum = 0
    best_test_solution = 1e7
    best_test_ratio = 0
    running_reward = 0
    critic_loss = []
    avg_length = 0
    ...
    print("Beginning Trainning: ", flush=True)
    # training loop
    for i_episode in range(1, args.max_episodes + 1):
        ...
        for b in range(args.batch_size):
            ...
            inp_lower_matrix, edge_candidates, ori_greedy, baselines, _ , _= tuples_train[graph_index]
            greedy = ori_greedy
        ...
        if i_episode % args.test_interval == 0:
            with torch.no_grad():
                ...
                # run testing
                test_dict = evaluate(ppo.policy, tsp_env, tuples_test, args.max_timesteps, args.search_size, mp_pool, args)
            ...
            if test_dict["ratio"]["mean"] > best_test_ratio:
                ...
                file_name = f'{args.checkpoint_save_dir}/PPO_{args.solver_type}_node{args.node_dimension}' \
                            f'_beam_{args.search_size}_ratio{best_test_ratio:.4f}.pt'
...
def attack_and_defense(args):
    ...
    sol_defense_filename = f'PPO_{args.solver_type}_node{args.node_dimension}' \
                    f'_beam_{args.search_size}_ratio'
    for file in os.listdir(args.pretrained_save_dir):
        if sol_defense_filename in file:
            sol_defense_filename = args.pretrained_save_dir + '/' + file
            print(f'The sol defense file name: {sol_defense_filename} \t')
            break
    sol_defense_policy = ActorCritic(*ac_params).to(device)
    sol_defense_state_dict = torch.load(sol_defense_filename)
    sol_defense_policy.load_state_dict(sol_defense_state_dict)
    for param in sol_defense_policy.parameters():
        param.requires_grad = False
        
    # load the attack model
    sol_attack_filename = f'PPO_{args.solver_type}attack_node{args.node_dimension}' \
                    f'_beam_{args.search_size}_ratio'
    for file in os.listdir(args.pretrained_save_dir):
        if sol_attack_filename in file:
            sol_attack_filename = args.pretrained_save_dir + '/' + file
            print(f'The sol attack file name: {sol_attack_filename} \t')
            break
    sol_attack_policy = ActorCritic(*ac_params).to(device)
    sol_attack_state_dict = torch.load(sol_attack_filename)
    sol_attack_policy.load_state_dict(sol_attack_state_dict)
    for param in sol_attack_policy.parameters():
        param.requires_grad = False
    ...
    print("########## Evaluate on Test ##########")
    sol_test_dict = evaluate(sol_attack_policy, tsp_env, tuples_test, args.max_timesteps, args.search_size, mp_pool, args = args, defense_net = sol_defense_policy, tester=tester)
    print("########## Evaluate complete ##########\n", flush=True)
...
def generate_hardcase(args):
    ...
    print("########## Evaluate on Test ##########")
    test_dict = evaluate(attack_policy, tsp_env, tuples_test, args.max_timesteps, args.search_size, mp_pool,
...
def parse_arguments():
    ...
    # environment configs
    parser.add_argument('--solver_type', default='furthest', choices=['nn','furthest', 'lkh-5', 'MatNet'])
    parser.add_argument('--resource_limit', default=600, type=float)
    parser.add_argument('--add_graph_features', action='store_true')
    parser.add_argument('--node_dimension', default=20, type=int)
    ...
    parser.add_argument('--test_sample', default=20, type=int, help='number of testing samples')

    # decode(testing) configs
    parser.add_argument('--search_size', default=3, type=int)
    ...
    # Bad case generation
    parser.add_argument('--is_record_bad_case', default=False, type=bool, help='Whether to record the bad case')

    # Defense setting
    parser.add_argument("--pretrained_save_dir", default="../pretrained/atsp", type=str, help='the direction to load the pretrained defense models')
    parser.add_argument("--is_defense", default=False, type=bool, help="run in attack and defense mode")
    parser.add_argument("--is_evaluate", default=False, type=bool, help='using evaluate mode')
    ...
    # Modify args path
    args.checkpoint_save_dir = args.checkpoint_save_dir + '_' + str(args.node_dimension) + '_' + args.solver_type
    args.tensorboard_save_dir = args.tensorboard_save_dir + '_' + str(args.node_dimension) + '_' + args.solver_type
    if args.is_attack:
...
if __name__ == '__main__':
    ...
    elif args.is_evaluate:
        generate_hardcase(args)
    else:
        main(args)