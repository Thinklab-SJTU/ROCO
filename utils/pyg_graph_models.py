...
class BipartiteGCN(nn.Module):
    def __init__(self, num_in_feats, num_out_feats, num_layers=3, batch_norm=True):
        ...
        self.num_out_feats = num_out_feats
        self.num_layers = num_layers
        reverse_in_feats = (num_in_feats[1], num_in_feats[0])
        new_in_feats = (num_out_feats, num_out_feats)
        
        for l in range(self.num_layers):
            if l == 0:
                conv_s2e = pyg.nn.SAGEConv(self.num_in_feats, self.num_out_feats)
                conv_e2s = pyg.nn.SAGEConv(reverse_in_feats, self.num_out_feats)
            else:
                conv_s2e = pyg.nn.SAGEConv(new_in_feats, self.num_out_feats)
                conv_e2s = pyg.nn.SAGEConv(new_in_feats, self.num_out_feats)
            if batch_norm:
                norm_s2e = nn.BatchNorm1d(self.num_out_feats)
                norm_e2s = nn.BatchNorm1d(self.num_out_feats)
            else:
                norm_s2e = nn.Identity()
                norm_e2s = nn.Identity()
            self.add_module('conv_s2e{}'.format(l), conv_s2e)
            self.add_module('norm_s2e{}'.format(l), norm_s2e)
            self.add_module('conv_e2s{}'.format(l), conv_e2s)
            self.add_module('norm_e2s{}'.format(l), norm_e2s)
        
        self.init_parameters()
    
    def init_parameters(self):
        for l in range(self.num_layers):
            nn.init.xavier_uniform_(getattr(self, 'conv_s2e{}'.format(l)).lin_l.weight)
            nn.init.xavier_uniform_(getattr(self, 'conv_s2e{}'.format(l)).lin_r.weight)
            nn.init.xavier_uniform_(getattr(self, 'conv_e2s{}'.format(l)).lin_l.weight)
            nn.init.xavier_uniform_(getattr(self, 'conv_e2s{}'.format(l)).lin_r.weight)
    
    def forward(self, *args):
        if len(args) == 1 and isinstance(args[0], (pyg.data.Data, pyg.data.Batch)):
            x, edge_index = args[0].x, args[0].edge_index
        elif len(args) == 2 and isinstance(args[0][0], torch.Tensor) and isinstance(args[1], torch.Tensor):
            x, edge_index = args
        else:
        ...
        for l in range(self.num_layers):
            reverse_x = (x[1], x[0])
            conv_s2e = getattr(self, 'conv_s2e{}'.format(l))
            conv_e2s = getattr(self, 'conv_e2s{}'.format(l))
            norm_s2e = getattr(self, 'norm_s2e{}'.format(l))
            norm_e2s = getattr(self, 'norm_e2s{}'.format(l))
            event_feat = conv_s2e(x, edge_index)
            event_feat = nn.functional.relu(norm_s2e(event_feat))
            strategy_feat = conv_e2s(reverse_x, reverse_edge_index)
            strategy_feat = nn.functional.relu(norm_e2s(strategy_feat))
            x = (strategy_feat, event_feat)
        return x
