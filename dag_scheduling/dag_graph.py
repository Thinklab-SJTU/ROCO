...
class DAGraph(object):
    def __init__(self, resource_dim, feature_dim, scheduler_type='sjf', is_attack = False):
    ...
    def step(self, input_graph, act, prev_greedy, defense = False):
        if self.is_attack and not defense:
            return self.step_Attack(input_graph, act, prev_greedy)
        ...
        edge_candidates = self.get_edge_candidates(new_graph, defense)
        done = all([len(x) == 0 for x in edge_candidates.values()])
        return reward, new_graph, new_greedy, edge_candidates, done

    def step_Attack(self, input_graph, act, prev_greedy):
        new_graph = input_graph.copy()
        if isinstance(act, torch.Tensor):
            act = (act[0].item(), act[1].item())
        # print("action", act[0], act[1])
        # edge_candidates = self.get_edge_candidates(new_graph)
        # print(edge_candidates[act[0]])
        new_graph.remove_edge(act[0], act[1],)
        assert nx.is_directed_acyclic_graph(new_graph)
        new_greedy = self.makespan_time(new_graph, self.scheduler_type)
        # # Positive
        # reward = prev_greedy - new_greedy
        ...
        reward = - prev_greedy + new_greedy
        edge_candidates = self.get_edge_candidates(new_graph)
        done = sum([len(x) for x in edge_candidates.values()]) == 0
        # print(done)
        return reward, new_graph, new_greedy, edge_candidates, done
    ...
    def get_edge_candidates(self, graph, defense = False):
        ...
        """
        if self.is_attack and not defense:
            return self.get_edge_candidates_Attack(graph)
    ...
    def get_edge_candidates_Attack(self, graph):
        """Candidates are node pairs that are not on any paths.
        This is not a bottle neck yet, but there is no need to repetitively call
        this whole process after adding one single edge.
        """
        ...
        parents, children, _ = self.get_dependency_nodes(graph)
        for node in graph.nodes(data=False):
            relations_map[node] = parents[node]

        edge_candidates = {}
        num_nodes = len(graph.nodes())
        for i in range(num_nodes):
            edge_candidates[i] = relations_map[i]
        # print()
