...
from dag_scheduler_model import ActorNet, CriticNet, GraphEncoder
...
from evaluate_dag import evaluate
...
def main(args):
    ...
    try:
        ...
        if key in env_dict and env_dict[key] == 'kubemaker':  # kubemaker mode
            ...
            if args.is_attack:
                tfboard_path = os.path.join(tfboard_path, "Attack")
        import socket
    ...
    for i_episode in range(1, args.max_episodes + 1):
        ...
        if i_episode % args.test_interval == 0:
            with torch.no_grad():
                ...
                # run testing
                test_dict = evaluate(ppo.policy, dag_graph, tuples_test, args.max_timesteps, args.search_size, mp_pool, args = args)
                # write to summary writter
                for key, val in test_dict.items():
                    if isinstance(val, dict):
                        if summary_writer and 'each' not in key:
            ...
            # ratio-mean is the reward/baseline. Attack or not, reward larger, the solution is better
            if test_dict["ratio"]["mean"] > best_test_ratio:
                best_test_ratio = test_dict["ratio"]["mean"]
                file_name = f'{args.checkpoint_save_dir}/PPO_{args.scheduler_type}_dag_num{args.num_init_dags}' \
                ...
                # cmd = 'LANG=C osscmd -H oss-cn-hangzhou-zmf.aliyuncs.com ' \
                #       '-i 7KOPGeG28ma3KLw8 -k qshD0xYiXPbqNICEAyRyYwfzKqyHL1 ' \
                #       f'put {file_name} oss://arks-model/{file_name} --replace=True'
                # try:
                #     subprocess.check_call(cmd, shell=True)
                # except subprocess.CalledProcessError as err:
                #     print('Warning: command failed with error:\n', err, file=sys.stderr)
...
def attack_and_defense(args):
    ...
    defense_filename = f'PPO_{args.scheduler_type}_dag_num{args.num_init_dags}' \
    ...
    for file in os.listdir(args.pretrained_save_dir):
        if defense_filename in file:
            ...
            print(f'The defense file name: {defense_filename} \t')
            break
    ...
    print("########## Evaluate on Test ##########")
    # train_dict = evaluate(attack_policy, dag_graph, tuples_train, args.max_timesteps, args.search_size, mp_pool, args=args, defense_net=defense_policy)
    test_dict = evaluate(attack_policy, dag_graph, tuples_test, args.max_timesteps, args.search_size, mp_pool, args = args, defense_net = defense_policy)
...
def parse_arguments():
    ...
    parser.add_argument('--scheduler_type', default='sft', choices=['sft', 'cp', 'ts'])
    parser.add_argument('--resource_limit', default=600, type=float)
    parser.add_argument('--add_graph_features', action='store_true')
    parser.add_argument('--num_init_dags', default=50, type=int)
    ...
    parser.add_argument('--max_timesteps', default=20, type=int, help='max timesteps in one episode')
    ...
    parser.add_argument('--node_output_size', default=64, type=int)
    parser.add_argument('--gnn_layers', default=5, type=int, help='number of GNN layers')
    ...
    parser.add_argument('--test_interval', default=100, type=int, help='run testing in the interval (episodes)')
    parser.add_argument('--log_interval', default=100, type=int, help='print avg reward in the interval (episodes)')
    parser.add_argument('--test_model_weight', default='../pretrained/dag/PPO_cpattack_dag_num50_beam5_ratio0.0926.pt',
                        type=str, help='the path of model weight to be loaded')

    # direction
    parser.add_argument('--checkpoint_save_dir', default='checkpoint/dag/', type=str, help='the direction to save the checkpoint')
    parser.add_argument('--tensorboard_save_dir', default='logs/tensorboard_log/dag/', type=str,
    ...
    # Defense setting
    parser.add_argument("--pretrained_save_dir", default="../pretrained/dag", type=str, help='the direction to load the pretrained defense models')
    ...
    # Modify args path
    args.checkpoint_save_dir = args.checkpoint_save_dir + '_' + str(args.num_init_dags) + '_' + args.scheduler_type
    args.tensorboard_save_dir = args.tensorboard_save_dir + '_' + str(args.num_init_dags) + '_' + args.scheduler_type
    if args.is_attack:
