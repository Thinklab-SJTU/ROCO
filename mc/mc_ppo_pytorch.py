...
def generate_hardcase(args):
    ...
    mp_pool = Pool(num_workers)
    print("########## Evaluate on Test ##########")
    test_dict = evaluate(attack_policy, mc_env, tuples_test, args.max_timesteps, args.search_size, mp_pool, args=args)
    print("########## Evaluate complete ##########\n", flush=True)

def parse_arguments():
    parser = argparse.ArgumentParser(description='MC solver. You have two ways of setting the parameters: \n'
                                                 '1) set parameters by command line arguments \n'
                                                 '2) specify --config path/to/config.yaml')
    # environment configs
    parser.add_argument('--solver_type', default='cbc',
                        choices=['greedy_naive', 'cbc', 'scip', 'gurobi'])
    parser.add_argument('--time_limit', default=0.5, type=float)
    ...
    parser.add_argument('--element_size', default=200, type=int)
    ...
    # direction
    parser.add_argument('--checkpoint_save_dir', default='checkpoint/0119', type=str,
                        help='the direction to save the checkpoint')
    parser.add_argument('--tensorboard_save_dir', default='logs/tensorboard_log/0119', type=str,
                        help='the direction to save the tensorboard')

    parser.add_argument('--is_record_hard_case', action='store_true', default=False, help='save hard cases')

    # Evaluate setting
    parser.add_argument("--pretrained_save_dir", default="../pretrained/mc", type=str, help="the direction to load the pretrained models")
    parser.add_argument("--is_evaluate", action='store_true', help='using evaluate mode')
    ...
    # Modify args path
    args.checkpoint_save_dir = args.checkpoint_save_dir + '_' + str(args.subset_size) + '_' + str(
        args.element_size) + '_' + args.solver_type
    args.tensorboard_save_dir = args.tensorboard_save_dir + '_' + str(args.subset_size) + '_' + str(
        args.element_size) + '_' + args.solver_type
